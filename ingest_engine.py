...
def get_time_period(days=None):
    ...
    - If 'days' is provided, returns the last N days.
    - If 'days' is None, returns the last full calendar month.
    """
...
def load_to_bigquery(records):
    if not records: print("No new records to load. Skipping BigQuery load."); return
...
def aggregate_and_publish():
    ...
    client = bigquery.Client(project=GCP_PROJECT_ID)
    query = f"""
        -- This query is unchanged but now runs from a single, reliable source.
    ...
    """
    job_config = bigquery.QueryJobConfig(use_query_cache=False) # Always get fresh data
    bq_results = client.query(query, job_config=job_config).result()

    locations_dict, all_timeframes = {}, set()
    for row in bq_results:
        # The original fix for NULL coordinates is now handled robustly by the query's WHERE clause
        lat, lon = round(row["latitude"], 5), round(row["longitude"], 5)
        key = (lat, lon)
        if key not in locations_dict:
            locations_dict[key] = {"latitude": lat, "longitude": lon, "stats": {}}
...
async def main(args):
    ...
    if args.latitude and args.longitude:
        ...
        async with aiohttp.ClientSession() as session:
            records = await fetch_one_location(session, float(args.latitude), float(args.longitude), period)
            all_records.extend(records)
    else:
        # --- MONTHLY INGESTION MODE ---
    ...
    # The workflow is now identical and simple: Fetch -> Load -> Aggregate
    if all_records:
...
if __name__ == "__main__":
    ...
    # Basic validation for backfill mode
